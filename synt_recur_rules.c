...
bool rule_EXPRESSION() {
    RLOG("<expression>\n");
    // TODO: return token back to scaner from stash, switch to precedence parser
    return true;
}
...
bool rule_FN_OR_EXP() {
    ...
    if ((t->type >= token_OP_START && t->type <= token_OP_END)
    || t->type == token_EOL) { // TODO: is it correct for every case?
